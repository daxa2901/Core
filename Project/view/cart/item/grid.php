...
<div class="container w-100 d-none" id="newItem">
	<form action="<?php echo  $this->getUrl('add');?>" method="POST" class="w-100 mx-auto">
		...
		<?php foreach ($products as $key => $value): ?>
				<tr>
					<td> <?php if($value->base): ?><img src="<?php  $value->getBase()->getImageUrl()?>" alt =  "no Image"  height="50px" width="50px" /> <?php else: ?> No Image <?php endif; ?></td>
					<td> <?php echo $value->name ?></td>
					<td> <input type="number" id = 'quantity' name = item[quantity][<?php echo $value->productId ?>] min = 1  max= <?php echo $value->quantity ?> value='1' onchange ="getRowTotal(this.value, <?php echo $value->price ?> , <?php echo $value->productId ?>)"></td>
		...
		<?php foreach ($items as $key => $value): ?>
			<?php $product = $value->getProduct(); ?>
				<tr>
					...
					<td> <input type="number" name="cart[quantity][<?php echo $value->itemId ?>]" id = 'quantity' min = 1  max= <?php echo $product->quantity ?> value="<?php echo $value->quantity ?>" onchange ="getRowTotal(this.value, <?php echo $product->price ?> , <?php echo $product->productId ?>)">
